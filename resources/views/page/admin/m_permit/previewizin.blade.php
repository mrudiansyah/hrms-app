...
<div style="border:1px solid #000;padding:5px 20px;">
	...
	<table style="width:100%;">
		<tr>
			...
			<td colspan="2" class="kecil kanan">NO.DOC: {{$nomor}}/SAI-OHCD/PERS/2014</td>
		</tr>
		<tr><td colspan="4" class="besar"><label class="judul">{{$dt->category}}</label> {{$judul}}</td></tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3" class="sedang garis">: {{$dt->employee_name}}</td>
		</tr>
		<tr>
			<td class="sedang">NIK</td>
			<td colspan="3" class="sedang garis">: {{$dt->NIK}}</td>
		</tr>
		<tr>
			<td class="sedang">BAGIAN</td>
			<td colspan="3" class="sedang garis">: {{$dt->dept_name}}</td>
		</tr>
		<tr><td colspan="4">&nbsp;</td></tr>
		<tr><td colspan="4" class="sedang">{{$konten}}</td></tr>
		<?php if($dt->category=='A'){?>
			...
			<tr>
				...
				<td colspan="3" class="sedang garis">: {{$dt->keperluan}}</td>
			</tr>
		<?php }?>
		...
		<?php if($dt->category=='D'){?>
			...
			<tr>
				...
				<td colspan="3" class="sedang garis">: {{$dt->berobat_ke}}</td>
			</tr>
			<tr>
				<td class="sedang">DARI PUKUL</td>
				<td colspan="2" class="sedang garis">: <?php echo date('H:i',strtotime($dt->start_izin))." <label class='seling'>S/D</label> ".date('H:i',strtotime($dt->finish_izin));?></td>
			...
			<td class="tengah">
				<?php if($dt->status_disetujui==1){
					...
					if (file_exists($filename)){?>
						<img src='{{ $ttd_app }}' style='height:30px'>
					<?php }else{?>
						<img src='{{ $confirm }}' style='width:50px;'>
					<?php }
				}elseif($dt->status_disetujui=='3'){?>
					<img src='{{ $reject }}' style='width:70px;'>
				<?php }?>
			</td>
		...
		<tr>
			<td class="tengah">
				<?php if($dt->status_personalia==1){
					...
					<img src='{{ $reject }}' style='width:50px;'>
				<?php }?>
			</td>
			<td style="height: 25px;">&nbsp;</td>
			<td colspan="2" class="tengah">
...
<div class="sedang">Created by: {{$dt->admin}} {{$dt->created_at}}</div>
