...
class training_controller extends Controller
{
    ...
    function index($type, $category)
    {
        if (request()->user()->hasRole('root') || request()->user()->hasRole('training')) {
            ...
            if ($type > 0)
                $tb_training_list = $tb_training_list->where('id_type', $type);
            if ($category != 'All Category')
                $tb_training_list = $tb_training_list->where('category', $category);
    ...
    function training_actual_group($type, $category)
    {
        if (request()->user()->hasRole('root') || request()->user()->hasRole('training')) {
            ...
            if ($type > 0)
                $tb_training_actual = $tb_training_actual->where('tb_training_list.id_type', $type);
            if ($category != 'All Category')
                $tb_training_actual = $tb_training_actual->where('tb_training_list.category', $category);
    ...
    function training_personal($id_employee = 0)
    {
        if (request()->user()->hasRole('root') || request()->user()->hasRole('training')) {
            $tb_training_personal = DB::table('tb_training_actual')
                ->leftjoin('tb_training_schedule', 'tb_training_schedule.id', '=', 'tb_training_actual.id_training_schedule')
                ->leftjoin('tb_training_list', 'tb_training_list.id', '=', 'tb_training_schedule.id_training')
                ->leftjoin('tb_skill_type', 'tb_skill_type.id', '=', 'tb_training_list.id_type')
                ->leftjoin('tb_training_participant', 'tb_training_participant.id_training_actual', '=', 'tb_training_actual.id')
    ...
    function simpan_list(request $data)
    {
        ...
        if ($data->idcomponent == '') {
            ...
            if ($tb_training_list > 0)
                $hasil = "Failed, Training Name already Exixts";
            else {
    ...
    function simpan_plan_participant(request $data)
    {
        ...
        foreach ($tb_employee as $dt) {
            ...
            if ($tb_training_invitation > 0)
                $hasil = "Failed, Employee already Exixts";
            else {
    ...
    function simpan_supporting(request $data)
    {
        ...
        if ($tb_related_document > 0)
            $hasil = "Failed, DDocument already Exixts";
        else {
    ...
    function simpan_examination(request $data)
    {
        ...
        $admin = Auth::user()->name;
        if ($data->testname == '')
            $hasil = "Test Name can not be null";
        elseif ($data->minutes == '')
            $hasil = "Minutes can not be null";
        elseif ($data->passinggrade == '')
            $hasil = "Passing Grade can not be null";
        else {
            if ($data->id == '') {
                $cek = DB::table('tb_training_test')->where('test_name', $data->test_name)->count();
                if ($cek > 0)
                    $hasil = "Test Name already exists";
                else {
    ...
    function simpan_question(request $data)
    {
        ...
        $admin = Auth::user()->name;
        if ($data->question == '')
            $hasil = "Question can not be null";
        elseif ($data->optiona == '')
            $hasil = "Option A can not be null";
        elseif ($data->answercode == '')
            $hasil = "Answer Code can not be null";
        else {
            if ($data->id == '') {
    ...
    function simpan_supporting_test(request $data)
    {
        ...
        if ($tb_related_test > 0)
            $hasil = "Failed, Test already Exixts";
        else {
                ...
                if ($add) {
                    ...
                    foreach ($tb_training_invitation as $dt) {
                        ...
                        if ($add2) {
                            ...
                            return "Failed Add Participant";
                    }
                } else
                    return "Failed Add Actual & Participant";
            }
        } else {
