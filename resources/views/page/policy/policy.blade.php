				...
				<div class="box box-primary" style="background:#FFF;">
					...
					<div class="box-body">
						...
						<table id="tables" class="table table-hover">
							...
							<tbody>
								<?php $no=0;?>
								@foreach($data['tb_policy'] as $dt)
								<tr>
									<td>
										<?php $no++;echo $no;?>
									</td>
									<td>{{$dt->policy_name}}</td>
									<td>{{$dt->description}}</td>
									<td>{{date('d F Y',strtotime($dt->publish_date))}}</td>
									<td>{{$dt->uploaded_by}}</td>
									<td>{{$dt->uploaded_at}}</td>
									<td>
										<a href="{{ route('policy.show', ['id' => $dt->id]) }}" class="btn btn-info btn-xs" target="_blank">
											<i class="fa fa-image"></i>
										</a>
										<button class="btn btn-primary btn-xs download-btn" data-id="{{$dt->id}}">
											<i class="fa fa-download"></i>
										</button>
										@if(request()->user()->hasRole('hr_access'))
											<button class="btn btn-success btn-xs" onclick="window.location.href='/PolicyLevel/{{$dt->id}}';">
												<i class="fa fa-user"></i>
											</button>
											<button class="btn btn-warning btn-xs upload-modal" data-id_file="{{$dt->id}}" data-policy_name="{{$dt->policy_name}}" data-description="{{$dt->description}}" data-publish_date="{{$dt->publish_date}}">
												<i class="fa fa-edit"></i>
											</button>
											<button class="btn btn-danger btn-xs nonactive" data-id="{{$dt->id}}">
												<i class="fa fa-upload"></i>
											</button>
											<button class="btn btn-danger btn-xs delete-modal" data-delid="{{$dt->id}}" data-delname="{{$dt->policy_name}}">
...
@section('Scripts')
	<script type="text/javascript">
		...
		$(document).on('click', '.download-btn', function() {
			...
			form.method = 'POST';
			form.action = "{{$site}}/Policy/Download";
