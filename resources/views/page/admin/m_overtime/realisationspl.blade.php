				...
				<div class="box box-primary" style="background:#FFF;">
					<div class="box-header">
						...
						<div class="box-tools pull-right">
							<?php if($submenu=='dept'){?>
								<a class='btn btn-app' href='/Admin/Overtime/Preview/{{$idspl}}' target="_blank"><i class='fa fa-print'></i>Print</a>
						...
						<table id="table2" class="table table-hover" style="min-width:100%;">
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="checkbox"<?php if($dt->sign_before==1)echo ' checked';?> class='fa fa-square-o confirm-modal' data-before='{{$dt->id}}'>
									</td>
									<td>
										<?php if($dt->sign_after=='1'||$dt->sign_after=='3')echo date('H:i',strtotime($dt->start_act)).' ~ '.date('H:i',strtotime($dt->finish_act));?>
									</td>
									<td title="{{$dt->hours_act}}">
										<?php 
										...
										if($dt->sign_before=='1'&&$dt->status_dicatat=='1'){
											...
											?>
												<div class="pull-right"><button type="button" class="btn btn-primary btn-xs update-modal" data-id_employee='{{$dt->id_employee}}' data-idafter='{{$dt->id}}' data-startact='{{$start_act}}' data-finishact='{{$finish_act}}' data-otcategory='{{$dt->ot_category}}' data-hoursact='{{$dt->hours_act}}' data-hoursconvertion='{{$dt->hours_convertion}}' data-otisoma='{{$dt->minutes_break}}'><i class="fa fa-edit"></i></button></div>
											<?php 
										}else {
											$signstate++;
					...
					<div class="box-footer">
						<?php if(isset($idspl)){?>
							<a href="/Admin/Overtime/Detail/Before/{{$idspl}}"><button type="button" class="btn btn-primary preview-btn">Sign All</button></a>
					...
					<div class="form-group" id="tambahan">
						...
						<select id="id_employee_baru" name="id_employee_baru" class="form-control selectpicker" data-live-search="true">
							...
							@foreach($tb_employee as $dt2)
								<option value="{{$dt2->id}}">{{$dt2->employee_name}} ({{$dt2->dept_code}})</option>
							@endforeach
