				...
				<div class="col-xs-12 col-md-12 col-lg-12">
					<div class="box box-primary" style="background:#FFF;">
						<div class="box-header">
							...
							<h3 class="box-title">MEMO {{$data['from_dept']}} ~ {{$data['to_dept']}}</h3>
							<div class="box-tools pull-right">
								...
								@if($data['status_draft']==1)
									...
									<button type="button" class="btn btn-success btn-md update_draft" data-id_memo="{{$data['id_memo']}}" data-status="0"><i class="fa fa-edit"></i> &nbsp;Confirm</button>
								@else
							...
							<table id="tables" class="table table-hover">
								...
								<tbody>
									...
									<tr <?php if($dt->status=='0')echo "style='background:#ebeeee;'";?>>
										...
										<td>{{$dt->jph_gsph}}</td>
										<td>{{$dt->process}}</td>
										<td>{{date('d-M-Y',strtotime($dt->date_ot))}}</td>
										<td>{{date('H:i',strtotime($dt->start_ot))}}</td>
										<td>{{date('H:i',strtotime($dt->finish_ot))}}</td>
										<td>{{$dt->plan_qty}}</td>
										<td>
											@if($dt->check_reason=='')
												<span class="label label-warning">{{$dt->reason_ot}}</span>
												<?php $salah++;?>
											@else
												{{$dt->reason_ot}}
											@endif
										...
										<td>
											...
											<div class="pull-right">
												@if($data['status_draft']==1||$dt->check_reason=='')
													<a href="{{ route('memos.edit', $dt->id) }}" class="btn btn-primary btn-xs">
														<i class="fa fa-edit"></i>
													</a>
												@endif
												@if($data['status_draft']=='1')
													<button class="btn btn-danger btn-xs delete-modal" data-delid="{{$dt->id}}" data-delid2="tb_memo_ot" data-delname="{{$dt->part_no}}">
				...
				<div class="row">
					<div class="col-xs-12 col-md-4 col-lg-4">
						<div class="box box-danger">
							<div class="box-header">
								<h3 class="box-title">DEPT. {{$data['from_dept']}}</h3>
							</div>
							<!-- /.box-header -->
					...
					<div class="col-xs-12 col-md-4 col-lg-4">
						<div class="box box-warning">
							<div class="box-header">
								<h3 class="box-title">DEPT. {{$data['to_dept']}}</h3>
							</div>
							<!-- /.box-header -->
							<div class="box-body table-responsive">
							<table class="table table-hover">
