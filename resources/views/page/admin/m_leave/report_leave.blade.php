										...
										<tr>
											...
											<td>
												...
												<?php if($dt->status_approved=='0'&&$dt->admin==$nama){?>
													<a href="/LeaveMail/{{$dt->id}}" target="_blank"><button title="Send Mail" type="button" class="btn btn-primary btn-xs"><i class="fa fa-envelope-o"></i></button></a>
													<button title="Delete" type="button" class="delete-modal btn btn-danger btn-xs" data-delid="{{$dt->id}}" data-delname="{{$dt->employee_name}} for {{$dt->start_leave}}"><i class="fa fa-trash"></i></button>
												<?php }?>
												</div>
											</td>
										</tr>
											...
											<td>
												...
												<div class="pull-right">
													<?php if($dt->status_legalized=='1'&&($dt->admin==$nama||request()->user()->hasRole('leave_legalize'))&&$dt->finish_leave<$Tgl){?>
														<button title="Confirm" type="button" class="approve-modal btn btn-success btn-xs" data-approveid="{{$dt->id}}" data-approvename="{{$dt->employee_name}}"><i class="fa fa-check-square-o"></i></button>
