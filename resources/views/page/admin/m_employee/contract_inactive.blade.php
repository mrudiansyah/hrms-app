					...
					<div class="box-header">
						...
						<div class="pull-right">
							...
							<?php }else{?>
								...
								<a href="/Status/NonActive/{{$periode}}" class="btn btn-app">
									<i class="fa fa-user-times"></i> Non Active
								</a>
								<a href="/Status/Arsif/{{$periode}}" class="btn btn-app">
					...
					<div class="box-body" style="overflow-x:scroll;">
						<div class="row">
							...
							<div class="col-xs-12 col-md-3 col-lg-2">
								<input class="form-control" type="month" id="periode" name="periode" value="{{$periode}}">
							</div>
						</div><br>
							...
							<tbody>
								...
								<tr <?php if($dt->finish_contract<=date('Y-m-d')&&$dt->finish_contract!=''&&$dt->contract_name!='Permanen') echo "style='background-color:#FF0;'";?>>
									<td>
										...
										?>
										<!-- <button type="button" class="btn btn-primary btn-xs update-modal" data-action='New' data-idcontract='<?php echo $dt->id;?>' data-idemployee='<?php echo $dt->idemployee;?>' data-nmemployee='{{$dt->employee_name}}' data-joindate='{{$dt->joindate}}' data-tglawal='{{$PeriodeAwal}}' data-statusawal='{{$dt->contract_name}}'><i class="fa fa-edit"></i></button> -->
									...
									<td>{{date('d-M-Y',strtotime($dt->joindate))}}</td>
									<td><?php if($Judul!='Draft Contract')echo $dt->contract_name;?></td>
									<td>
										<?php if($Judul!='Draft Contract'){if($dt->contract_name=='Permanen'||$dt->contract_name=='Draft')echo "&nbsp;";else echo date('d-M-Y',strtotime($dt->start_contract));}?>
										<?php if($Judul=='Employee (Draft)'){?>
											<div class="pull-right">
												<a href="/Status/Deactive/{{$dt->idemployee}}" type="button" class="btn btn-danger btn-xs"><i class="fa  fa-user-times"></i></a>
											</div>
										<?php }?>
										<?php if($Judul=='Employee (Non Active)'){?>
											<div class="pull-right">
												<a href="/Status/Reactive/{{$dt->idemployee}}" type="button" class="btn btn-primary btn-xs"><i class="fa fa-check"></i></a>
												<?php
													$markup=date('Y-m-d',strtotime('+2 months',strtotime($dt->start_contract)));
													$today=date('Y-m-d');
													if($markup<$today){?>
													<a href="/Status/Delete/{{$dt->idemployee}}" type="button" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a>
												<?php }?>
											</div>
										<?php }?>
									</td>
...
@section('Scripts')
	...
	<script type="text/javascript">
		$('body').on("change","#periode",function(){
			...
			window.location.href="/Status/NonActive/"+periode;
		});
	</script>
